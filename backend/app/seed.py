...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS interest_only_rules(id INTEGER PRIMARY KEY, name TEXT NOT NULL, interest_only_months INTEGER NOT NULL, enabled INTEGER NOT NULL DEFAULT 1, created_at TEXT);
    """)
    ...
    if conn.execute("SELECT COUNT(*) c FROM interest_only_rules").fetchone()["c"] == 0:
        conn.execute("INSERT INTO interest_only_rules(name,interest_only_months,enabled,created_at) VALUES ('只息12期',12,1,datetime('now'))")
        conn.commit()
    conn.close()
